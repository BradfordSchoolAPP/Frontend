...
export default class AddBenefitScreen extends React.Component {
    constructor(props){
        super(props)
     ...
     _validate(){
        ...
        if(this.state.name === '' || this.state.details === '' || this.state.date === '' || this.state.deadline_date === '' || this.state.deadline_hour=== '' || this.state.requeriments.length=== 0 ){
            ...
            this.setState({loading:true, confirm:false, message:"Enviando información"})
            this.showAlert()
            this.send()
        }
    });
    ...
    send(){
      ...
      setTimeout(()=>{this.setState({loading:false, confirm:true, message:"Beca creada exitosamente"});}, 2000);
      }
    ...
    addRequeriment(){
        ...
        console.log("VOY A CREAR EL REQUISITO:", requeriment.name)
        this.setState({newRequeriments:auxNewRequeriments}, function () {
  ...
  render(){
    ...
    return(
        <View style={styles.container}>
            <Header {...this.props} namePage="Nueva beca"/> 
            ...
            <View style={styles.boton}>
                <TouchableHighlight style={styles.bottom} onPress={() => this._validate()}>
                    <Text style={styles.text}>Crear beca</Text>
                </TouchableHighlight>
            </View>  
